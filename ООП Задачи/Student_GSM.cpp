...
public:
	...
	void getYMD(string& egn[], int& year, int& month, int& day)
	{
		year = atoi(egn[0]) * 10 + atoi(egn[1]);
		month = atoi(egn[2]) * 10 + atoi(egn[3]);
		day = atoi(egn[4]) * 10 + atoi(egn[5]);
...
public:
	...
	virtual ostream& OutputGSM(ostream& toStream, GSM& p) const
	{
